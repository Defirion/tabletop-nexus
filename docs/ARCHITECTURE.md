...
The lifecycle regression suite uses a tiny original fixture runtime that consumes the real Nexus launch environment, binds the assigned private endpoint, serves the fixed readiness surface, records graceful termination, can deliberately ignore `SIGTERM`, can trickle an unfinished readiness response, and can deliberately fail readiness or crash. Synthetic signal-delivery failures additionally exercise the lease-retention boundary without depending on a host-specific OS failure mode.
