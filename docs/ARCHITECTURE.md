...
The proxy resolves the decoded game ID only against the configured registry and routes only when that same game is the supervisor's active `running` runtime. The browser-facing game prefix is removed and the remaining path/query is forwarded to the private listener. Request and response bodies are streamed rather than buffered, so ordinary HTTP bodies, SSE, and other long-lived responses retain their transport behavior. WebSocket upgrades use the same route resolution and private runtime port; Node socket piping supplies backpressure, and Nexus removes WebSocket extension negotiation rather than enabling compression implicitly.
...
Nexus performs this decision on the raw request target before WHATWG URL dot-segment normalization. It percent-decodes once, rejects malformed or nested encoding, controls, backslashes, duplicate separators, and `.`/`..` segments, then compares the complete first post-prefix segment with an ASCII-only case fold. The validated segments are re-encoded into the backend request target. This gives HTTP and WebSocket routing one path interpretation and fails closed on encoded separators, encoded traversal, and mixed-case/encoded management aliases while leaving distinct whole segments such as `__nexusx` and `__nexus-status` game-owned.
