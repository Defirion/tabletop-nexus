...
For the local launcher, the supervisor-facing exit promise represents definitive process termination: a successful child's post-launch `error` event is not treated as exit evidence, while `close` is the completion point used for lease ownership. Spawn failure detail is retained until that close. If `SIGTERM` or the forced `SIGKILL` cannot be delivered and the child is not already known to have exited, stop fails without releasing the lease or admitting a replacement runtime.
...
Only one runtime is active initially. Starting another game serially stops the current runtime, waits for termination, releases its port lease, and only then allocates/launches the replacement. On Linux the local launcher sends `SIGTERM` first and uses `SIGKILL` after the grace period if necessary. The supervisor installs its definitive-exit observer immediately after launch, so a startup or stop cleanup failure retains the lease while the process remains live but a later confirmed exit still releases the lease and active slot. Unexpected process exit transitions the game to `failed` and releases the lease once exit is known.

The lifecycle regression suite uses a tiny original fixture runtime that consumes the real Nexus launch environment, binds the assigned private endpoint, echoes the per-launch readiness token, serves the fixed readiness surface, records graceful termination, can deliberately ignore `SIGTERM`, can trickle an unfinished readiness response, can emit output beyond ordinary pipe capacity before binding, and can deliberately fail readiness or crash. A deterministic racing responder occupies the assigned port with an otherwise valid status payload carrying the wrong token, proving Nexus observes but never accepts that endpoint. Synthetic signal-delivery failures additionally exercise the lease-retention boundary without depending on a host-specific OS failure mode.
