...
test("npm and canonical verification use explicit product-verifier modes", async () => {
  const [packageText, verifier] = await Promise.all([read("package.json"), read("verify.ps1")]);
...
test("legacy pre-bootstrap canonical invocation cannot certify a newer target", () => {
  ...
  assert.match(`${result.stdout}\n${result.stderr}`, /requires explicit --local or --canonical-target <40-character SHA> mode/);
});
...
test("canonical verifier requires R1 runtime sources and focused regression coverage", async () => {
  const [verifier, productVerifier] = await Promise.all([read("verify.ps1"), read("scripts/verify-product.mjs")]);
  ...
  for (const path of [
    ...
    "src/runtime/process-launcher.js",
    "test/private-ports.test.js",
    "test/process-launcher.test.js",
  ]) {
