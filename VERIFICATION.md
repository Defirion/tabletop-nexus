...
The R1 test suite includes real loopback fixture-process coverage for launch environment, launch-token endpoint association, fixed readiness, absolute readiness deadlines, lifecycle transitions, startup cleanup, one-active-game switching, unexpected exit, supervised-output backpressure, and Linux graceful/forced shutdown behavior. Linux-specific process-group regressions also exercise runtime-owned helpers: graceful helper cleanup, forced switching where root and helper both ignore `SIGTERM`, and unexpected root exit with deliberately blocked descendant cleanup. Those tests assert the private-port lease and replacement slot remain owned until no live helper remains. Linux-only shutdown/process-group tests skip on Windows; the cross-platform implementation remains syntax/unit checked there.
